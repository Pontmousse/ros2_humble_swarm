'''
This file is taken from marvelmind company for using the Super beacons
'''
...
class MarvelmindHedge (Thread):
    def __init__ (self, adr=None, tty="/dev/ttyACM0", baud=115200, maxvaluescount=3, debug=False, recieveUltrasoundPositionCallback=None, recieveImuRawDataCallback=None, recieveImuDataCallback=None, recieveUltrasoundRawDataCallback=None):
        self.tty = tty  # serial
    ...
    def print_raw_imu(self): 
        ...
        if (ri[10]):
            ...
            tmsec= int(ri[9] % 1000)
            dt = datetime.datetime.utcfromtimestamp(tsec)
            print ("Hedge {:d} Raw IMU: AX:{:d}, AY:{:d}, AZ:{:d},   GX:{:d}, GY:{:d}, GZ:{:d},   MX:{:d}, MY:{:d}, MZ:{:d},   at time T: {:%Y-%m-%d %H:%M:%S}-{:03d}".format(ri[0], ri[1], ri[2], ri[3], ri[4], ri[5], ri[6], ri[7], ri[8], ri[9], dt, tmsec))
        else:
            pass
            print ("Hedge {:d} Raw IMU: AX:{:d}, AY:{:d}, AZ:{:d},   GX:{:d}, GY:{:d}, GZ:{:d},   MX:{:d}, MY:{:d}, MZ:{:d},   at time T: {:.3f}".format(ri[0], ri[1], ri[2], ri[3], ri[4], ri[5], ri[6], ri[7], ri[8], ri[9], ri[10]/1000.0))
...
#                           print(bufferList)
                            ...
                            try:
                                if (len(bufferList) > pktHdrOffset + 4 + msgLen + 2):
                                    ...
                                    if (isCmMessageDetected):
                                        ...
                                        ax, ay, az, gx, gy, gz, mx, my, mz, imuAdr, timestamp, usnCRC16 = struct.unpack_from ('<hhhhhhhhhBxxxxxLxxxxH', strbuf, pktHdrOffset + 5)
                                    elif (isNTRawImuMessageDetected):
                                        ax, ay, az, gx, gy, gz, mx, my, mz, imuAdr, timestamp, usnCRC16 = struct.unpack_from ('<hhhhhhhhhBxxxxxqxxxxH', strbuf, pktHdrOffset + 5)
                                    ...
                                    if CRC_calc == usnCRC16:
                                        if (isMmMessageDetected or isCmMessageDetected or isNTMmMessageDetected):
                                            ...
                                            if (self.recieveUltrasoundRawDataCallback != None):
                                            ...
                                            if (self.recieveImuDataCallback != None):
                    ...
                    print ('*** ERROR: OS error (possibly serial port != available)')
                    time.sleep(3)
